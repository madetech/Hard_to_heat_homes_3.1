...
function initMap() {
    ...
    for (const prop of mapData.props) {
        ...
        const score = Number(prop.score) >=1 && Number(prop.score) <= 4 ? Number(prop.score) : null;

        const marker = L.marker([prop.lat, prop.long], {
            icon: score !== null ? scoreIcons[score] : scoreIcons[5],
        }).addTo(map);

        markersDict[prop.uprn] = marker;

        marker.on("mouseover", function () {
            if (score !== null) marker.setIcon(scoreIconsHover[score]);
        });

        marker.on("mouseout", function () {
           if (score !== null) marker.setIcon(scoreIcons[score]);
        });
